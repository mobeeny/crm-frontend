...
import {
  setSelectedClientId,
  setSelectedClient,
} from "../redux/reducers/selectedClient";
import {
...
export default function DetailsLayout() {
  ...
  const selectedTab = useSelector(
    (state) => state.uiControls.detailsSelectedTab
  );
  const dispatch = useDispatch();

  const selectedClient = useSelector(
    (state) => state.selectedClient.selectedClient
  );

  const handleTabChange = (event, newValue) => {
    dispatch(setdetailsSelectedTab(newValue));
    // if (newValue === 1)
    //   dispatch(setSelectedCompanyId(selectedClient.company[0]));
  };
        ...
        <Grid
          ...
          <Tabs
            ...
            <Tab
              icon={<CompanyIcon />}
              label="Company"
            //   disabled={!selectedClient.company}
            />
            <Tab
              icon={<OrderIcon />}
              label="Orders"
